...
// Corrected import (naming it paymentService for clarity)
import { getPaymentSettings } from "../services/paymentService";

export default function Step5Submit({ formData }) {
  const [showChargeModal, setShowChargeModal] = useState(false);
  const [fee, setFee] = useState(0); // State to store the fee from backend
  const navigate = useNavigate();

  // Load the current fee settings from the backend
  useEffect(() => {
    const fetchFee = async () => {
      try {
        const settings = await getPaymentSettings();
        setFee(settings.processing_fee || 0);
      } catch (error) {
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="bg-amber-50 border border-amber-100 p-4 rounded-2xl flex gap-3 items-start">
        ...
        <p className="text-xs text-amber-800 font-medium leading-relaxed">
          ...
          A processing fee of <strong>₹{fee}</strong> is required to initiate verification.
        </p>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="bg-indigo-600 p-8 text-center text-white relative">
              ...
              {/* Corrected: Using the 'fee' state variable */}
              <h3 className="text-indigo-100 mt-2 uppercase tracking-widest">Processing Fee: <span className="text-bold rounded-4xl  text-bold text-green-600 b-2 bg-white p-3">₹{fee}</span></h3>
            </div>

            <div className="p-8 text-center space-y-6">
              <div className="space-y-2">
                <p className="text-slate-500 text-sm font-medium leading-relaxed">
                  To proceed with your loan verification, a one-time refundable processing charge is applicable.
                </p>
              </div>

              <div className="bg-slate-50 p-4 rounded-2xl flex items-center justify-center gap-2">
                <FaCheckCircle className="text-emerald-500" />
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Secure Razorpay Gateway</span>
              </div>
